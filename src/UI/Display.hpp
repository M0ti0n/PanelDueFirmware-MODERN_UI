...
public:
	ModernStopButton(PixelNumber py, PixelNumber px, PixelNumber pw, PixelNumber ph,
		event_t e, LcdFont pf = nullptr);

	void Refresh(bool full, PixelNumber xOffset, PixelNumber yOffset) override;
};

// Rail ALERT button: a red vector warning triangle on the rail background.
class ModernAlertNavButton : public SingleButton
{
private:
	PixelNumber height;
...
public:
	ModernAlertNavButton(PixelNumber py, PixelNumber px, PixelNumber pw, PixelNumber ph,
		event_t e);

	void Refresh(bool full, PixelNumber xOffset, PixelNumber yOffset) override;
};

// Master navigation tiles (CONTROL / STATUS / SYSTEM). The glyph is vector-drawn and the
// tile is filled with the current Accent colour while it is the active tab.
enum class MasterNavIcon : uint8_t
{
	Joystick,
	List,
	Gear
...
class ModernMasterNavButton : public SingleButton
{
private:
	MasterNavIcon icon;
	PixelNumber height;
...
public:
	ModernMasterNavButton(PixelNumber py, PixelNumber px, PixelNumber pw, PixelNumber ph,
